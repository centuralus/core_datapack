...
execute if entity @s[scores={barter=1..}] run tellraw @s [{"nbt":"playerdb.player.data.centural.core.offer","storage":"rx:io"},"\n",{"text":"Item found in offer"}]
...
execute if entity @s[scores={barter=0}] at @s run data modify entity @e[tag=claim,distance=..1,limit=1,type=minecraft:armor_stand] HandItems[1] set from storage rx:io playerdb.player.data.centural.core.offer
execute if entity @s[scores={barter=0}] at @s run data modify storage rx:io playerdb.player.data.centural.core set value {}
