...
execute if entity @p[scores={wild=2..}] as @a[scores={wild=2..}] if entity @s[scores={wild_timer=-2000..0}] run tellraw @s {"text":"Cooldown active ~ Please wait approximately 2 mins before use","color":"yellow"}

execute if entity @p[scores={wild=2..}] as @a[scores={wild=2..}] unless entity @s[scores={wild_timer=-2000..0}] run tag @s add wild

execute if entity @p[scores={wild=2..}] as @a[scores={wild=2..}] unless entity @s[scores={wild_timer=-2000..0}] run scoreboard objectives add wild_timer minecraft.custom:minecraft.play_time


execute if entity @p[tag=wild] as @a[tag=wild] run scoreboard players set @s wild_timer -2000

execute if entity @p[tag=wild] as @a[tag=wild] in minecraft:overworld run tp @s @e[tag=spawn,limit=1]
